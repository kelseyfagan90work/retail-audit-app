...
export function buildEmailSubject({ storeName, templateName, auditPeriod, completedAt }) {
  const monthSource = auditPeriod || completedAt;
  const [year, month] = monthSource.slice(0, 7).split('-');
...
export async function sendReportEmail({ to, subject, html, attachments }) {
  ...
  const fromAddress = process.env.REPORT_FROM_EMAIL || 'onboarding@resend.dev';
